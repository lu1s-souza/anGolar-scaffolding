...
func GetProjects(c *fiber.Ctx) error {
	homePath := getUserHome()
	if projectsDir, err := os.ReadDir(homePath + "/projects"); err == nil {
		for _, projects := range projectsDir {
			fmt.Printf("entry name: %s, is Dir: %t", projects.Name(), projects.IsDir())
		}
	} else {
		fmt.Println(err)
	}
...
// @Security ApiKeyAuth
// @Router /v1/book [post]
func GetProjectModules(c *fiber.Ctx) error {
	// Get now time.
	// now := time.Now().Unix()
...
// @Security ApiKeyAuth
// @Router /v1/book [put]
func GetModuleData(c *fiber.Ctx) error {
	// Get now time.
	// now := time.Now().Unix()
...
func GetModuleComponents(c *fiber.Ctx) error {
	// Get now time.
	// now := time.Now().Unix()
...
// @Security ApiKeyAuth
// @Router /v1/book [delete]
func GetModuleServices(c *fiber.Ctx) error {
	// Get now time.
	// now := time.Now().Unix()
...
func GetModuleTests(c *fiber.Ctx) error {
	// Get now time.
	// now := time.Now().Unix()
...
func GetModuleStore(c *fiber.Ctx) error {
	// Get now time.
	// now := time.Now().Unix()
